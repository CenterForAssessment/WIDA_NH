##########################################################################################
###                                                                                    ###
###   WIDA New Hampshire Learning Loss Analyses -- 2020 Baseline Growth Percentiles    ###
###                                                                                    ###
##########################################################################################
...
if (length(tmp.names <- grep("BASELINE|SS", names(WIDA_NH_SGP_LONG_Data))) > 0) {
		WIDA_NH_SGP_LONG_Data[,eval(tmp.names):=NULL]
}
...
#####
###   Run BASELINE SGP analysis - create new WIDA_NH_SGP object with historical data
#####

###   Temporarily set names of prior scores from sequential/cohort analyses
data.table::setnames(WIDA_NH_SGP_LONG_Data,
	c("SCALE_SCORE_PRIOR", "SCALE_SCORE_PRIOR_STANDARDIZED"),
	c("SS_PRIOR_COHORT", "SS_PRIOR_STD_COHORT"))

SGPstateData[["WIDA_NH"]][["Assessment_Program_Information"]][["CSEM"]] <- NULL

WIDA_NH_SGP <- abcSGP(
        sgp_object = WIDA_NH_SGP_LONG_Data,
...
data.table::setnames(WIDA_NH_SGP@Data,
